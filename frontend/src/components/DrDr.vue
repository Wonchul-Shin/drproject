<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            DrID : {{ decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]) }}
        </v-card-title>
        <v-card-title v-else>
            Dr을 생성합니다.
        </v-card-title>
        ...
        <v-card-text style="background-color: white;">
            ...
            <label class="v-label">유형</label>
            <String v-model="value.type" :editMode="editMode" :inputUI="''" />
            <!-- <v-select v-model="value.type" :items="['신뢰적DR', '자발적DR']" :editMode="editMode" /> -->
        </v-card-text>
        <v-card-actions style="background-color: white;">
            ...
            <div>
                ...
                <v-btn color="primary" text @click="remove" v-else>
                    종료하기
                </v-btn>
